...
function SWEP:DealDamage()
	...
	if ( SERVER && IsValid( tr.Entity ) ) then
		...
		if ( IsValid( phys ) ) then
			if tr.Entity:GetSaveTable()["m_iName"]:sub(1,10) == "volleyball" then
				phys:ApplyForceOffset( self.Owner:GetAimVector() * 2500 * phys:GetMass(), tr.HitPos )
			else
				phys:ApplyForceOffset( self.Owner:GetAimVector() * math.random(120,300) * phys:GetMass(), tr.HitPos )
			end
		end
	end
